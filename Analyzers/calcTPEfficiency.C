...
void calcTPEfficiency()
{
   ...
   **************************************************/
  const char *c_ZMCPath = "../Data/TagAndProbe/Inputs/DY_NoRegion_RI_005.root";
  const char *c_DataPath = "../Data/TagAndProbe/Inputs/Data_TP_NoRegion_RI_005.root";
  ...
  **************************************************/
   ...
   **************************************************/
  // MC
  fit_mc[0] = new TF1("MC_Z_Iso_All", "gaus(0) + gaus(3)", 60, 120);   //!- Iso
  fit_mc[1] = new TF1("MC_Z_Iso_Pass", "gaus(0) + gaus(3)", 60, 120);  //
  fit_mc[2] = new TF1("MC_Z_Iso_Fail", "gaus(0) + gaus(3)", 60, 120);  //
  
  fit_mc[3] = new TF1("MC_Z_ID_Pass", "gaus(0) + gaus(3)", 60, 120); //!--- ID
  fit_mc[4] = new TF1("MC_Z_ID_Fail", "gaus(0) + gaus(3)", 60, 120); //
  
  // Data
  fit_data[0] = new TF1("Data_Z_Iso_All", "gaus(0) + gaus(3)", 60, 120);   //!- Iso
  fit_data[1] = new TF1("Data_Z_Iso_Pass", "gaus(0) + gaus(3)", 60, 120);  //
  fit_data[2] = new TF1("Data_Z_Iso_Fail", "gaus(0) + gaus(3)", 60, 120);  //
  
  fit_data[3] = new TF1("Data_Z_ID_Pass", "gaus(0) + gaus(3)", 60, 120); //!--- ID
  fit_data[4] = new TF1("Data_Z_ID_Fail", "gaus(0) + gaus(3)", 60, 120); //
  ...
  cout << "MC ID (Pass/(Pass+Fail)): " << mcTagPass_ID/(mcTagPass_ID+mcTagFail_ID) << endl;
  cout << "MC Iso (Pass/(Pass+Fail)): " << mcTagPass_Iso/(mcTagPass_Iso+mcTagFail_Iso) << endl << endl;

  cout << "Data Efficiencies):" << endl;
  cout << "Data ID (Pass/(Pass+Fail)): " << dataTagPass_ID/(dataTagPass_ID+dataTagFail_ID) << endl;
  cout << "Data Iso (Pass/(Pass+Fail)): " << dataTagPass_Iso/(dataTagPass_Iso+dataTagFail_Iso) << endl << endl;
   ...
   *  Write Histograms                              *
   **************************************************/
  TFile *f_histos = new TFile("../Data/TagAndProbe/Outputs/TP_Histos_RI_005_v2.root","RECREATE");
  f_histos->cd();
  for (int i = 0; i != 3; ++i)
  {
    h_mcIso[i]->Write();

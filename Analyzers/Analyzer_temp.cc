...
//  nstream <<  name << ".root";
   nstream <<  "/uscms/home/jwwetzel/WorkingArea/Majorana/CMSSW_5_3_8_LQ/test/LQanalyzer/DataFileOutputs_R04_WPU_V7/" << name << ".root";
...
void Analyzer::Loop()
{
  ...
  for (Long64_t jentry = 0; jentry < entrieslimit; jentry++ ) {
    ...
    */  
	...
	if ( muonTColl[m1].charge()*muonTColl[m2].charge() > 0
	     && muonTColl[m1].lorentzVec().Pt() >=20 ) {
...
//       }
    ...
    cuts[23] = cuts[22] && cuts[20] && cuts[21] && cuts[19] && cuts[18]; //muon jet vertex mu mass 
...
//	  cout << "Found the same jet matches both 1: " << j1[0] << " 2: " << j2[0] << endl;
  ...
  if (debug) cout<< "out of the loop" <<endl;
   ...
   fstream dataOutFile;
   dataOutFile.open("/uscms/home/jwwetzel/WorkingArea/Majorana/CMSSW_5_3_8_LQ/test/LQanalyzer/Results/EventCounts_Dxy_2_RelIso_0_10_R04.txt",fstream::app|fstream::out);
//   dataOutFile << "REPLACEME1" << endl;
//   dataOutFile << "dxy: 2.0" << endl;
   for(unsigned int i=0; i!=ePerCut.size(); ++i){
      cout << CutName[i] << ": " << ePerCut[i] << " " << ePerCutPU[i] << " " << sEvents[i] << " " << endl;//sEvents[i]/totalE << endl;
//      dataOutFile << CutName[i] << "\t";// << ePerCut[i] << "\t" << ePerCutPU[i] << "\t" << sEvents[i] << "\t" << endl;//sEvents[i]/totalE << endl;
   }
   for(unsigned int i=0; i!=ePerCut.size(); ++i){
      cout << CutName[i] << ": " << ePerCut[i] << " " << ePerCutPU[i] << " " << sEvents[i] << " " << endl;//sEvents[i]/totalE << endl;
      dataOutFile << ePerCut[i] << "\t";// << ePerCutPU[i] << "\t" << sEvents[i] << "\t" << endl;//sEvents[i]/totalE << endl;
   }
   cout << endl;
   dataOutFile << endl;
   dataOutFile.close();

  ...
  CutName(numCuts, "")
{
  ...
  /**********************************************
   ***    Event counters for specific cuts.   ***
   **********************************************/
  CutName[0] = "Rel_Iso";
...
void Analyzer::WriteRoot()
{
  ...
  {
    cout << "Error.  Did not write histograms." << endl;
  }
}
...
void Analyzer::SetEvtN(Long64_t events)
{
  ...
  (entrieslimit == -1) ? (cout << "Will run All events." << endl) : (cout << "Will run " << entrieslimit << " events." << endl);
}

/**********************************************
 ***      Set if running on MC or not       ***
 **********************************************/
...
/****************************************************
 ***  Set if running on MC, and if so what mass   ***
 ****************************************************/
...
/**********************************************
 ***            Set Systematics.            ***
 ***      Generally set one at a time       ***
 ***          SetSys(1,0,0,0,0,0)           ***
 ***    Or    SetSys(0,1,0,0,0,0)...        ***
 **********************************************/
...
{
  /**********************************************
   ***        Check fChain and outfile        ***
   **********************************************/
  if (fChain == 0)
  {
    cout << "There are no events!" << endl;  //!-- No events!
    return;
  }
  if(outfile->IsWritable())
  {
    cout << "Outfile is writeable.  Starting loop." << endl;
  }
  else
  {
    cout << "ERROR: Outfile is not writeable.  Quitting." << endl;
    return;
  }
  
  
  /**********************************************
   ***    Tag and Probe Selection Criteria    ***
   **********************************************/
  //  Probe
  double d_probeMuonPt1 = 20.0; //!-- Muon PT 1
  double d_probeMuonPt2 = 20.0; //!-- Muon PT 2
  double d_probeEta     = 2.4; //!--- Muon Eta
  double d_probeChiNDoF = 20.0; //!-- Muon ChiSquare # Degrees of Freedom
  double d_probeBSDxy   = 0.2; //!--- Muon BSdXY, Impact Parameter
  double d_probeDz      = 1.0; //!--- Muon Dz
  double d_probeEcalIso = 100.0; //!- Muon Ecal Isolation
  double d_probeHcalIso = 100.0; //!- Muon Hcal Isolation
  double d_probeTrkHits = 11.0; //!-- Muon Tracker Hits
  double d_probeRelIso  = 100.0; //!- Muon Relative Isolation
  
  //  Tag
  double d_tagMuonPt1 = 20.0; //!-- Muon PT 1
  double d_tagMuonPt2 = 20.0; //!-- Muon PT 2
  double d_tagEta     = 2.4; //!--- Muon Eta
  double d_tagChiNDoF = 10.0; //!-- Muon ChiSquare # Degrees of Freedom
  double d_tagBSDxy   = 0.005; //!- Muon BSdXY, Impact Parameter
  double d_tagDz      = 0.1; //!--- Muon Dz
  double d_tagEcalIso = 4.0; //!--- Muon Ecal Isolation
  double d_tagHcalIso = 6.0; //!--- Muon Hcal Isolation
  double d_tagTrkHits = 11.0; //!-- Muon Tracker Hits
  double d_tagRelIso  = 0.05; //!-- Muon Relative Isolation

  float puw = 1;  //!-------------- PileUp weighting.
   ...
   **********************************************/
  if( !entrieslimit || fChain->GetEntries() < entrieslimit || entrieslimit < 0)
    entrieslimit = fChain->GetEntries();
  cout << "Starting: "  << entrieslimit << " of " << fChain->GetEntries() << endl;
  
  
  /**********************************************
   ***       Begin running over entries       ***
   **********************************************/
  for (Long64_t jentry = 0; jentry < entrieslimit; jentry++ )
  {
    puw = 1;
    
    if (!(jentry % 10000))  //!-- Spit out progress every 10,000 events.
    ...
    /**********************************************
     *** Find a good vertex.  We only need one. ***
     **********************************************/
    ...
    /*** Set Probe Muon Selection Criteria  ***/
    MuonTight.SetPt2(     d_probeMuonPt2  );
    MuonTight.SetPt1(     d_probeMuonPt1  );
    MuonTight.SetEta(     d_probeEta      );
    MuonTight.SetRelIso(  d_probeRelIso   );
    MuonTight.SetChiNdof( d_probeChiNDoF  );
    MuonTight.SetBSdxy(   d_probeBSDxy    );
    MuonTight.SetDz(      d_probeDz       );
    MuonTight.SetCalIso(  d_probeEcalIso, d_probeHcalIso);
    MuonTight.SetTrkHits( d_probeTrkHits  );
    std::vector<int> pcuts = MuonTight.MuonSelection(*MuonIsPF, *MuonIsGlobal, *MuonEta, *MuonPhi, *MuonPt, *MuonPtError, *MuonEnergy, *MuonPFIsoR03ChargedHadron, *MuonPFIsoR03NeutralHadron, *MuonPFIsoR03Photon, *MuonEcalVetoIso, *MuonHcalVetoIso, *MuonCharge, *MuonGlobalTrkValidHits, *MuonTrkPixelHits, *MuonStationMatches, *MuonTrackLayersWithMeasurement, *MuonGlobalChi2, *MuonTrkVx, *MuonTrkVy, *MuonTrkVz, *MuonTrkD0, *MuonTrkD0Error, VertexX->at(VertexN), VertexY->at(VertexN), VertexZ->at(VertexN), *MuonPFIsoR03PU, muonPColl); //!-- RelIso 03
    //std::vector<int> pcuts = MuonTight.MuonSelection(*MuonIsPF, *MuonIsGlobal, *MuonEta, *MuonPhi, *MuonPt, *MuonPtError, *MuonEnergy, *MuonPFIsoR04ChargedHadron, *MuonPFIsoR04NeutralHadron, *MuonPFIsoR04Photon, *MuonEcalVetoIso, *MuonHcalVetoIso, *MuonCharge, *MuonGlobalTrkValidHits, *MuonTrkPixelHits, *MuonStationMatches, *MuonTrackLayersWithMeasurement, *MuonGlobalChi2, *MuonTrkVx, *MuonTrkVy, *MuonTrkVz, *MuonTrkD0, *MuonTrkD0Error, VertexX->at(VertexN), VertexY->at(VertexN), VertexZ->at(VertexN), *MuonPFIsoR04PU, muonPColl); //!-- RelIso 04

    /*** Set Tag Muon Selection Criteria  ***/
    MuonTight.SetPt2(     d_tagMuonPt2  );
    MuonTight.SetPt1(     d_tagMuonPt1  );
    MuonTight.SetEta(     d_tagEta      );
    MuonTight.SetRelIso(  d_tagRelIso   );
    MuonTight.SetChiNdof( d_tagChiNDoF  );
    MuonTight.SetBSdxy(   d_tagBSDxy    );
    MuonTight.SetDz(      d_tagDz       );
    MuonTight.SetCalIso(  d_tagEcalIso, d_tagHcalIso);
    ...
    if(muonTColl.size())
    {
      ...
      /**********************************************
       ***      Loop, if at least 1 tag muon      ***
       **********************************************/
      for(int ti=0; ti!=muonTColl.size(); ++ti)
      {
         ...
         **********************************************/
        for(int pi=0; pi!=muonPColl.size(); ++pi)
        {
          ...
          int reg = ((pm.Perp() < 20) ? 0 : 1); //!------------ PT Region Binning.  Since cut is already 20, there is only 1 region.
          h_TPMass[0][reg]->Fill((tm+pm).M()); //!------------- Save T+P mass
          int pix = muonPColl[pi].ilepton(); //!--------------- pix is the current probe muon
          bool id=false;
          bool iso=false;
          bool win=false;
          ...
          /**********************************************
           ***   Probe Muon Relative Isolation check  ***
           **********************************************/
          if ( 1 && muonPColl[pi].relIso() < 0.05)
            iso = true;
          
          /**********************************************
           ***     Ensure T+P muons add to Z mass     ***
           **********************************************/
          ...
          if(id) //!----------------------------------------- If Pass ID Histograms
          {
            h_TPMass[1][reg]->Fill((tm+pm).M(), puw);
            if(win) tpCount[1][reg] += puw;
            if(iso) //!-------------------------------------- If Pass ID and Iso Histograms
            {
            ...
            else //!----------------------------------------- If Pass ID and Fail Iso
            {
              h_TPMass[5][reg]->Fill((tm+pm).M(), puw);
              if(win) tpCount[5][reg] += puw;
            }
          }
          else if(iso) //!----------------------------------- If Fail ID and Pass Iso
          {
            h_TPMass[2][reg]->Fill((tm+pm).M(), puw);
  ...
  if(outfile->IsWritable())
    cout << "Success! Loop can write output." << endl;
...
//    JetsVeto.SetPt(20);
//    JetsVeto.SetEta(2.4);
//    JetsVeto.JetSelectionLeptonVeto(*PFJetPassLooseID, *PFJetEta, *PFJetPhi, *PFJetPt, *PFJetEnergy, *PFJetNeutralEmEnergyFraction, *PFJetNeutralHadronEnergyFraction, *PFJetChargedEmEnergyFraction, *PFJetChargedHadronEnergyFraction, *PFJetChargedMultiplicity, *PFJetNConstituents, *PFJetCombinedSecondaryVertexBTag, electronTightColl, muonTColl, jetCollVeto);
//
...
//        for(int ji=1; ji<PFJetPt->size(); ++ji)
//        {
...
//   
//   dataOutFileFull.open("/uscms/home/jwwetzel/WorkingArea/Majorana/CMSSW_5_3_8_LQ/test/LQanalyzer/Results/Final/baseline_FULL.txt",fstream::app|fstream::out);
//   dataOutFileFull << "REPLACEME1" << endl;
//   
//   dataOutFileAppended.open("/uscms/home/jwwetzel/WorkingArea/Majorana/CMSSW_5_3_8_LQ/test/LQanalyzer/Results/Final/baseline_APPENDED.csv",fstream::app|fstream::out);
...
void Analyzer::MCLoop()
{
  ...
  int msi = 0;
  int ni = 0;
  for(int i=0; i!=GenParticlePx->size(); ++i)
  {

...
cat > rootiamo_job${clusterNumber}.C<<EOF
{
  ...
  if (1)
  {
    TChain* chain = ChainMaker("/uscms/home/jwwetzel/WorkingArea/Majorana/CMSSW_5_3_8_LQ/test/LQ_Analyzer/Runners/InputDataSetLists/TPData_4000.txt");

    Analyzer Pippo;
    Pippo.Init(chain);

    Pippo.SetName("TP_Data_4000_jets");
    std::cout << "TP_Data_4000_jets\n";
    Pippo.SetEvtN(-1);
